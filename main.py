...
from getQRcode import shot_new
...
def find(image, image_name, contours, hierachy, root=0):
    ...
    print(box)
    # cv2.drawContours(result, [box], 0, (255, 255, 255), 2)
    ...
    # cv2.imshow('findImg', image)
    cv2.waitKey(0)
    # cv2.imshow('resImg', result)
    cv2.waitKey(0)
    path = os.path.join(img_result, image_name)
    # cv2.imwrite(path, result)
    cv2.imencode(".jpg", result)[1].tofile(path)
    try:
        shot_new(img_path="results/" + image_name, left_up=box[0], left_down=box[3], right_up=box[2], image_name=image_name)
...
if __name__ == '__main__':
    ...
    for file in files:
        image = cv2.imdecode(np.fromfile(os.path.join(img_path, file), dtype=np.uint8), cv2.IMREAD_COLOR)
